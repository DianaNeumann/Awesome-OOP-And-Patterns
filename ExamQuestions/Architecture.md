...
>
> Сложная модель более адекватно представляет запутанную бизнес-логику, но труднее поддается отображению в реляционную схему базы данных. В простых моделях подчас достаточно применять варианты тривиального типового решения активная запись (Active Record), в то время как в сложных без замысловатых преобразователей данных (Data Mapper) порой просто не обойтись

> • `Table Module` представляет собой "привлекательный островок вблизи экватора, разделяющего враждебные земли **Transaction Script** и **Domain Model**" (c).
---
