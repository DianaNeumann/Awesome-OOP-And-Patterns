...
> 1) Тупо отобразить таблицу базу данных в какую-нибудь простую структуру данных, типа **Map**. Тогда записи в таблице будет сопоставлен словарик с key-value. Такое себе решение.  
> 
...
> `abstract: Связывание классов друг с другом происходит не на этапе компиляции кода, а во время его выполнения. Plugin позволяет делать это централизованно, без необходимости редактирования и сборки исходного кода для изменения конфигурации. Применяется тогда, когда одно и то же поведение может иметь несколько реализаций в зависимости от выбранной исполняющей среды`

> Сначала стоит сказать, зачем это надо. У нас есть паттерн `Separated Interface `, который предполагает размещение интерфейса и его реализации в разных пакетах (модулях). Помимо очевидных плюсов, в виде замены различных реализаций, мы также можем следовать правилам о том, как классы из одного пакета могут обращаться к классам из другого пакета. Разумеется, чтобы подобное приложение заработало во время выполнения, интерфейс должен иметь некоторую реализацию. Например, то, которое запрещает классам с уровня данных обращаться к классам с уровня представления. Для этого можно воспользоваться отдельным пакетом, который будет связывать интерфейс и реализацию во время компиляции, или же связатьих во время настройки приложения посредством паттерна `Plugin`. 

> Паттерн **Separated Interface** часто используется, когда один код выполняется в нескольких средах и требует разной реализации отдельной логики. Большинство разработчиков добиваются этого при помощи использования шаблона фабрики. Представим, что нам нужно генерировать первичный ключ при помощи паттерна **Separated Interface**. Окей, ну давайте использовать для юнит-тестирования простой объект-счёчик, а на реальной системе - последовательность из БД. Фабричный метод скорее всего будет содержать условный переход (if), проверяющий, установлен ли флаг тестирования, и возвращать необходимый генератор ключа. 
>
> Но тут проблема: как только у вас появится ещё несколько фабрик - начнётся путаница. Добавление новых конфигураций среды
развертывания, например "запуск юнит-тестов на БД без контроля транзакций" или "запуск в продакшн на DB2 с полной поддержкой транзакций", потребует правок в условиях в большом количестве фабрик, пересборку и переразвёртывание
>
...
> Важные моменты: 
> - Вначале нужно определить отделенный интерфейс для каждого поведения, которое требует применения различных реализаций в зависимости от выбранной исполняющей среды
> - Будем пользоваться разновидностью паттерна **Фабрика**, несколько видоизменив его в соответствии с нашими требованиями.
> -  Реализация Plugin'а в виде объекта-фабрики требует, чтобы все правила связывания находились в единственной внешней точке. Это необходимо для облегчения настройки приложения. Кроме того, связывание интерфейса с реализацией должно выполняться не во время компиляции, а динамически, т.е. во время выполнения, чтобы избежать повторной сборки приложения.
> - Правила связывания можно хранить в обычном текстовом файле (типа appsettings.json). Метод-фабрика Plugin'а считывает файл настроек, ищет строку, задающую реализацию запрошенного интерфейса и возвращает эту реализацию.

> ![image](https://user-images.githubusercontent.com/56086653/213325587-87ca0e21-98e9-4ef9-bc1d-3e7edcfeaa7f.png)
>
> ![image](https://user-images.githubusercontent.com/56086653/213325634-44ce1fd2-aac9-4dbe-99e6-0b571eb9dbbe.png)
>
> ![image](https://user-images.githubusercontent.com/56086653/213325677-bd7f6c81-e475-4400-ac8d-54c87e7f595e.png)
>
> ![image](https://user-images.githubusercontent.com/56086653/213325707-4768b11c-d5ba-4cb1-b73b-b308e399449b.png)
>
> ![image](https://user-images.githubusercontent.com/56086653/213325735-37104b1b-c244-4bb6-bcd0-1a172deb3cb4.png)
