...
> Опастность заключается в том, что простая реализация Singleton применима только в однопоточном режиме. Для потокобезопасности используется подход **Double Check Lock**.
>
